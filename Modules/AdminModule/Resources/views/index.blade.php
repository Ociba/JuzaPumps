                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                           @foreach($get_all_clients as $i =>$clients)
                                            <tr>
                                            @php
                                                if( $get_all_clients->currentPage() == 1){
                                                    $i = $i+1;
                                                }else{
                                                    $i = ($i+1) + 10*($get_all_clients->currentPage()-1);
                                                }
                                            @endphp
                                            <th scope="row">{{$i}}</th> 
                                                ...
                                                <td>
                                                    ...
                                                    <a href="/clientmodule/edit-clients-info/{{$clients->id}}"  class="btn btn-success mb-1 btn-sm waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="Edit Client Information">Edit</a>
                                                    @endif
                                                    @if(in_array('Can Trash Client', auth()->user()->getUserPermisions()))
                                                    <a href="/clientmodule/trash/{{$clients->id}}" class="btn btn-danger btn-sm mb-1 waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="Trash Client">Trash</a>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
